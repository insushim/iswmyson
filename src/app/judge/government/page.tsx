...
import { Landmark, ShoppingCart, BookOpen, Store, ArrowLeft, Search } from 'lucide-react'
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" onClick={() => router.push('/judge/dashboard')}>
          ...
          {selectedShop ? (
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1">주문 내용 (이유)</label>
                  <Textarea
                ...
                <Button
                  ...
                  disabled={isOrdering}
                >
                  {isOrdering ? <LoadingSpinner size="sm" className="mr-2" /> : <Landmark className="mr-2 h-4 w-4" />}
                  정부 주문 확정
                </Button>
              </CardContent>
            </Card>
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600">{shop.owner.nickname} | {shop.jobTitle}</p>
                          {shop.description && (
        ...
        <TabsContent value="ledger" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>정부가 가게에 지급한 모든 내역</CardDescription>
            </CardHeader>
            <CardContent>
              {orders.length === 0 ? (
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-gray-500">({order.ownerNickname})</span>
                            </div>
                            <p className="text-sm text-gray-600 mt-1">{order.description}</p>
                            <p className="text-xs text-gray-400 mt-1">{formatDate(order.createdAt)}</p>
                          </div>
                          <MarkDisplay amount={order.amount} className="text-red-600" />
                        </div>
                      </CardContent>
                    </Card>
                  ))}
