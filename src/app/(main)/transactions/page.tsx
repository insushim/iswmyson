...
import { ArrowUpRight, ArrowDownLeft, Send, Check, X, AlertTriangle, Coins } from 'lucide-react'
...
export default function TransactionsPage() {
  ...
  const handleObjection = async () => {
    ...
    try {
      const response = await fetch(`/api/transactions/${objectionForm.transactionId}/objection`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setSendForm({ ...sendForm, receiverId: e.target.value })}
              >
            ...
            <div>
              <label className="block text-sm font-medium mb-1">금액 (마크)</label>
              <Input
                type="number"
          ...
          <CardFooter>
            <Button className="w-full" onClick={handleSend} disabled={isSending}>
              {isSending ? <LoadingSpinner size="sm" className="mr-2" /> : <Coins className="mr-2 h-4 w-4" />}
              송금 요청
            </Button>
          </CardFooter>
        </Card>
                  ...
                  <Card key={tx.id}>
                    <CardContent className="py-4">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <p className="font-medium">{tx.sender.nickname}님이 보냄</p>
                          {tx.description && (
                  ...
                  <Card key={tx.id}>
                    <CardContent className="py-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-medium">{tx.receiver.nickname}님에게</p>
                          {tx.description && (
                            <p className="text-sm text-gray-600">{tx.description}</p>
                          )}
                        </div>
                    ...
                    return (
                      <Card key={tx.id}>
                        <CardContent className="py-4">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              <MarkDisplay
                                amount={tx.amount}
                                size="lg"
                                className={isSender ? 'text-red-500' : 'text-green-500'}
                              />
                              {tx.hasObjection && (
                          ...
                          {canObject && objectionForm.transactionId !== tx.id && (
                            <Button
                              size="sm"
                              variant="outline"
                              className="mt-3 text-green-600 border-green-600"
                              onClick={() => setObjectionForm({ transactionId: tx.id, reason: '' })}
                            >
                          ...
                          {objectionForm.transactionId === tx.id && (
                            <div className="mt-3 p-3 bg-green-50 rounded-lg space-y-2">
                              <Textarea
                                ...
                                onChange={(e) => setObjectionForm({ ...objectionForm, reason: e.target.value })}
                                placeholder="이의 제기 사유를 입력해주세요"
                                rows={2}
                              />
                              <div className="flex gap-2">
                                <Button size="sm" variant="success" onClick={handleObjection}>
                                  제출
                                </Button>
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => setObjectionForm({ transactionId: '', reason: '' })}
                                >
                                  취소
                                </Button>
                              </div>
                        ...
                        <CardContent className="py-4">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              ...
                              <Badge variant="secondary" className="mt-1">거절됨</Badge>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
